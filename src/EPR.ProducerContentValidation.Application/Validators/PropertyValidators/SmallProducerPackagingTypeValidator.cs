...
public class SmallProducerPackagingTypeValidator : AbstractValidator<ProducerRow>
{
    ...
    public SmallProducerPackagingTypeValidator()
    {
        ...
        // Valid packaging types (WasteType). CLR is excluded here as it has its own specific error (911).
        RuleFor(x => x.WasteType)
            .IsInAllowedValues(_validPackagingTypes)
            .When(x => x.ProducerSize == ProducerSize.Small && x.WasteType != PackagingType.ClosedLoopRecycling)
            .WithErrorCode(ErrorCode.PomFileSmallOrganisationSizePackagingTypeInvalidErrorCode);

        // CLR is not a valid packaging type for small organisations.
        RuleFor(x => x.WasteType)
            .IsNotInValues(new List<string> { PackagingType.ClosedLoopRecycling }.ToImmutableList())
